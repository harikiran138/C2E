...
export default function InstitutionOnboarding() {
    ...
    const init = async () => {
      ...
      if (institution) {
        ...
        setInstDetails({
          ...
          institution_status:
            (institution.institution_status as any) || "Non-Autonomous",
          established_year:
            institution.established_year || new Date().getFullYear(),
  ...
  const handleSaveDetails = async () => {
    ...
    if (
      ...
      setErrorMsg(
        `Established year must be a valid year between 1800 and ${currentYear + 1}.`,
      );
      return;
    }
  ...
  const handleAddProgram = async () => {
    ...
    if (
      ...
      !newProgram.level ||
      newProgram.duration === "" ||
      !newProgram.program_code
    ) {
      setErrorMsg(
        "Please fill all required fields (Name, Degree, Level, Duration, Code)",
      );
      return;
    }
    ...
    if (
      programs.some(
        (p) => p.program_code === newProgram.program_code.toUpperCase(),
      )
    ) {
          ...
          <div className="space-y-12">
            ...
            <div className="space-y-4">
              {[
                ...
                <div
                  key={s.step}
                  className={`flex items-center gap-4 transition-all duration-500 ${s.active ? "opacity-100 translate-x-1" : "opacity-20 translate-x-0"}`}
                >
                  <div
                    className={cn(
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="space-y-8">
                        ...
                        <div className="space-y-4">
                          <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2 px-1">
                            <Layers className="size-4 text-primary" /> Programs
                            ({programs.length})
                          </h4>
                          <div className="space-y-3">
                            {programs.map((p, idx) => (
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="p-4 bg-background/40 border border-border/40 rounded-xl">
                                  ...
                                  <p className="text-sm font-medium text-slate-800 line-clamp-3">
                                    {instDetails.vision || "Not specified"}
                                  </p>
                                </div>
                                <div className="p-4 bg-background/40 border border-border/40 rounded-xl">
                                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2">
                                  ...
                                  <p className="text-sm font-medium text-slate-800 line-clamp-3">
                                    {instDetails.mission || "Not specified"}
                                  </p>
                                </div>
                              </div>
                            </div>
