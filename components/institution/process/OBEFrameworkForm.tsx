...
export default function OBEFrameworkForm() {
  ...
  const handleAddDocument = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("obe-framework").getPublicUrl(filePath);
    ...
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center rounded-[2rem] border-2 border-dashed border-slate-200 bg-slate-50/50">
        ...
        <p className="text-slate-500 max-w-sm mt-2 font-medium">
          Please select a program from the dashboard to view the OBE Framework.
        </p>
      </div>
    );
  }
          ...
          {loading ? (
              ...
              <AnimatePresence>
                {filteredItems.map((item, idx) => (
                  ...
                  >
                    ...
                    <h4 className="text-lg font-black text-slate-900 mb-2 line-clamp-2 leading-tight">
                      {item.title}
                    </h4>
                    <p className="text-sm font-medium text-slate-500 mb-6 line-clamp-3 leading-relaxed flex-grow">
                      {item.description ||
                        "No description provided for this protocol document."}
                    </p>
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center bg-slate-50/50 rounded-[2.5rem] border border-dashed border-slate-200">
              ...
              <p className="text-slate-500 font-medium max-w-sm">
                No books or framework documents found for this program.
              </p>
            </div>
          )}
        </div>
