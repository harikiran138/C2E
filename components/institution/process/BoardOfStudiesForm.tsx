  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/institution/bos?programId=${programId}`,
      );
      if (response.ok) {
    ...
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center rounded-[2rem] border-2 border-dashed border-slate-200 bg-slate-50/50">
        ...
        <p className="text-slate-500 max-w-sm mt-2 font-medium">
          Please select a program from the dashboard to constitute the Board of
          Studies.
        </p>
      </div>
    );
        ...
        <div className="flex items-center justify-between mb-8 border-b border-slate-100 pb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 font-medium">
              Board of Studies membership management
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[1.5rem] border border-slate-200 border-dashed text-center">
              ...
              <p className="text-sm text-slate-500 max-w-xs mt-1 mb-6">
                The Board of Studies for this program hasn't been constituted
                yet.
              </p>
              <button
                        ...
                        <div className="px-5 pb-6 pt-0 border-t border-slate-100/80 bg-slate-50/30">
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-6">
                            ...
                            <InfoCard
                              ...
                              label="Tenure"
                              value={
                                member.tenure_start_date
                                  ? `${new Date(member.tenure_start_date).toLocaleDateString()} - ${member.tenure_end_date ? new Date(member.tenure_end_date).toLocaleDateString() : "Present"}`
                                  : null
                              }
                              secondary
                            />
