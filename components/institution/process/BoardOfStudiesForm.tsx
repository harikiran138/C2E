...
const BOS_CATEGORIES = [
  ...
  'Nominee of the VC (Affiliating University)',
  'Industry/Corporate',
  'Alumni',
  'UGC Nominee',
  'Member Secretary'
...
function BoSFormContent() {
  ...
  const fetchMembers = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/institution/bos?programId=${programId}`);
      if (response.ok) {
  ...
  const handleEdit = (member: BoSMember, e: React.MouseEvent) => {
    e.stopPropagation();
    setFormData({
    ...
    setExpandedId(member.id);
     ...
     // Scroll to form
     const formElement = document.getElementById('bos-form');
     if (formElement) {
         formElement.scrollIntoView({ behavior: 'smooth', block: 'center' });
     }
  };

  const handleDelete = async (member: BoSMember, e: React.MouseEvent) => {
    e.stopPropagation();
    if (!confirm('Are you sure you want to delete this member?')) return;
    
    try {
        const response = await fetch(`/api/institution/bos?id=${member.id}`, {
            method: 'DELETE',
        });
        if (response.ok) {
            fetchMembers();
  ...
  const handlePrintPDF = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Board of Studies (BoS)</title>
          <style>
        ...
        <body>
          <h1>Constitution of Board of Studies (BoS)</h1>
          <div class="header-info">Institutional Resource Planning Report - Department Level</div>
          <table>
            <thead>
              <tr>
            ...
            <tbody>
              ${members.map((m, i) => `
                <tr>
                  ...
                  <td>
                    ${m.tenure_start_date ? new Date(m.tenure_start_date).toLocaleDateString() : '-'}<br/>to<br/>
  ...
  if (!programId) {
    return (
        <div className="flex flex-col items-center justify-center p-20 text-center rounded-[2rem] border-2 border-dashed border-slate-200 bg-slate-50/50">
            ...
            <p className="text-slate-500 max-w-sm mt-2 font-medium">Please select a program from the dashboard to constitute the Board of Studies.</p>
        </div>
    );
  }
  ...
  return (
    <div className="space-y-8 animate-element">
      {/* Main Form Section */}
      <div id="bos-form" className="group rounded-[1.5rem] border border-slate-200 bg-white p-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] transition-all hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)]">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Category</label>
                 <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="text-xs font-medium">BoS member classification</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
              </div>
          ...
          <div className="flex gap-4 pt-6 border-t border-slate-100 mt-6">
            ...
            {!isLocked && members.length > 0 && (
              ...
              >
                <Lock className="size-5 text-slate-400" /> Lock BoS
              </button>
            )}
      ...
      <div className="space-y-4 pt-4">
        <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-slate-500 mb-4 px-1">
           <UserCog className="size-4 text-indigo-500" /> BoS Committee Members 
        ...
        <div className="grid grid-cols-1 gap-4">
            {loading ? (
                <div className="flex flex-col justify-center items-center py-16 bg-white rounded-2xl border border-slate-200 border-dashed">
                    ...
                    <p className="text-sm font-medium text-slate-400">Loading committee...</p>
                </div>
            ) : members.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[1.5rem] border border-slate-200 border-dashed text-center">
                    <div className="size-16 rounded-full bg-slate-50 flex items-center justify-center mb-4">
                        <Gavel className="size-8 text-slate-300" />
                    </div>
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                    <div className="md:col-span-4">
                                        ...
                                        <div className="text-xs font-bold text-indigo-600 uppercase tracking-wide mt-0.5">{member.member_id || 'No ID'}</div>
                                    </div>
                                    ...
                                    >
                                        <div className="px-5 pb-6 pt-0 border-t border-slate-100/80 bg-slate-50/30">
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-6">
                                                ...
                                                <Phone className="size-4 text-slate-400 mr-2" />
                                                ...
                                                <InfoCard 
                                                    icon={Calendar} 
                                                    label="Tenure" 
                                                    value={member.tenure_start_date ? `${new Date(member.tenure_start_date).toLocaleDateString()} - ${member.tenure_end_date ? new Date(member.tenure_end_date).toLocaleDateString() : 'Present'}` : null} 
                                                    secondary 
                                                />
...
export default function BoardOfStudiesForm() {
  return (
    <Suspense fallback={
        <div className="flex flex-col justify-center items-center py-20">
            <Loader2 className="size-10 animate-spin text-indigo-600 mb-4" />
            <p className="text-slate-500 font-medium">Initializing Board of Studies...</p>
        </div>
    }>
      <BoSFormContent />
    </Suspense>
