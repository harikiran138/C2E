...
export default function ProgramCoordinatorForm() {
  ...
  return (
    <div className="space-y-8 animate-element">
      {/* Main Form Section */}
      <div id="coordinator-form" className="group rounded-[1.5rem] border border-slate-200 bg-white p-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] transition-all hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)]">
        <div className="flex items-center justify-between mb-8 border-b border-slate-100 pb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 font-medium">Manage program coordinators and their permissions</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 gap-x-8 gap-y-6 md:grid-cols-2">
              ...
              <div className="flex items-center gap-1.5">
                 ...
                 <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p className="text-xs font-medium">Select academic designation</p>
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
              </div>
            ...
            {/* Program Field */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Program</label>
              <div className="relative">
                <select 
            ...
            {/* Personal Email */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Personal Email</label>
              <input 
                type="email" 
            ...
            {/* Official Mobile */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Official Mobile</label>
              <input 
            ...
            {/* Personal Mobile */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-widest text-slate-500">Personal Mobile</label>
              <input 
      ...
      <div className="space-y-4 pt-4">
        <h3 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-slate-500 mb-4 px-1">
           ...
           <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full text-xs ml-2">{coordinators.length}</span>
        </h3>
        ...
        <div className="grid grid-cols-1 gap-4">
            {loading ? (
                <div className="flex flex-col justify-center items-center py-16 bg-white rounded-2xl border border-slate-200 border-dashed">
                    ...
                    <p className="text-sm font-medium text-slate-400">Loading coordinators...</p>
                </div>
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[1.5rem] border border-slate-200 border-dashed text-center">
                    ...
                    <h4 className="text-lg font-bold text-slate-900">No Coordinators Yet</h4>
                    <p className="text-sm text-slate-500 max-w-xs mt-1 mb-6">Start by adding a program coordinator to manage curriculum and governance.</p>
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                    <div className="md:col-span-4">
                                        <div className="font-bold text-slate-900 truncate text-lg">{coordinator.name}</div>
                                        <div className="text-xs font-bold text-indigo-600 uppercase tracking-wide mt-0.5">{coordinator.designation}</div>
                                    </div>
                                    
                                    <div className="md:col-span-4 hidden md:block">
                                        <div className="flex items-center gap-2 text-sm text-slate-600 bg-slate-50 px-3 py-1.5 rounded-lg w-fit">
                                            <BookOpen className="size-3.5 text-slate-400" />
                                            <span className="truncate max-w-[200px] font-medium">{coordinator.program_name || 'No Program'}</span>
                                        </div>
                                    </div>
                                    ...
                                    >
                                        <div className="px-5 pb-6 pt-0 border-t border-slate-100/80 bg-slate-50/30">
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-6">
                                                <InfoCard icon={Mail} label="Official Email" value={coordinator.email_official} />
                                                <InfoCard icon={Phone} label="Official Mobile" value={coordinator.mobile_official} />
                                                <InfoCard icon={Linkedin} label="LinkedIn" value={coordinator.linkedin_id} isLink />
                                                
                                                <InfoCard icon={Mail} label="Personal Email" value={coordinator.email_personal} secondary />
                                                <InfoCard icon={Phone} label="Personal Mobile" value={coordinator.mobile_personal} secondary />
