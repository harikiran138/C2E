...
export default function AcademicCouncilForm() {
  ...
  const handleDelete = async (member: any, e: React.MouseEvent) => {
    ...
    if (
      !confirm(
        "Are you sure you want to delete this member from the Academic Council?",
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `/api/institution/academic-council?id=${member.id}`,
        {
          method: "DELETE",
        },
      );
        ...
        <div className="flex items-center justify-between mb-8 border-b border-slate-100 pb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1 font-medium">
              Add and Manage Academic Council (AC) Members
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[1.5rem] border border-slate-200 border-dashed text-center">
              ...
              <p className="text-sm text-slate-500 max-w-xs mt-1 mb-6">
                Constituent members for the Academic Council haven't been added
                yet.
              </p>
              <button
