...
function PeoPoMatrixContent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // 1. Fetch Program Details (for Matrix)
                ...
                // 2. Fetch PEOs
                const peoRes = await fetch(`/api/institution/peos?programId=${programId}`);
                if (peoRes.ok) {
                ...
                // 3. Fetch POs
                const poRes = await fetch(`/api/institution/program-outcomes?programId=${programId}`);
                if (poRes.ok) {
    ...
    if (loading) return (
        <div className="flex flex-col justify-center items-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
            ...
            <p className="text-slate-500 font-medium">Initialising PEO-PO Mapping...</p>
        </div>
    );
        ...
        <div className="space-y-8 animate-element pb-20">
            ...
            <div className="relative group rounded-[2rem] border border-slate-200 bg-white p-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] transition-all hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)]">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">PEO to PO Mapping</h2>
                            ...
                            <tr className="bg-slate-50/80 backdrop-blur-sm border-b border-slate-100">
                                <th className="px-8 py-6 min-w-[300px]">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">PEOs / Outcomes</span>
                                    </div>
                                </th>
                                {pos.map((po, i) => (
                                    <th key={po.id} className="px-4 py-6 text-center min-w-[80px] border-l border-slate-100/50">
                                        <TooltipProvider>
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <div className="cursor-help cursor-pointer">
                                                        <div className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">{po.po_code}</div>
                                                    </div>
                                                </TooltipTrigger>
                                                <TooltipContent side="top" className="max-w-xs p-3 rounded-xl border-indigo-100 shadow-xl">
                                                    <p className="text-xs font-semibold leading-relaxed text-slate-700">{po.po_description}</p>
                                                </TooltipContent>
                                            </Tooltip>
                                        </TooltipProvider>
                                    </th>
                                ...
                                <tr>
                                    <td colSpan={pos.length + 1} className="py-20 text-center">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-slate-400 font-bold italic">No PEOs found.</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                peos.map((peo, peoIdx) => (
                                    <tr key={peo.id} className="hover:bg-slate-50/50 transition-colors group/row">
                                        <td className="px-8 py-5">
                                            <div className="flex gap-4">
                                                ...
                                                <p className="text-sm font-semibold text-slate-700 leading-relaxed pt-1.5 truncate max-w-[250px]">{peo.peo_statement}</p>
                                            </div>
                                        </td>
                                        {pos.map((po) => (
                                            <td key={po.id} className="px-2 py-5 border-l border-slate-50">
                                                <div className="flex justify-center">
                                                    <select
                                                        disabled={isLocked}
                                                        value={matrix[`${peo.id}_${po.po_code}`] || '-'}
                                                        onChange={(e) => handleCellChange(peo.id, po.po_code, e.target.value)}
                                                        className={cn(
                                                            "w-14 h-9 text-center text-xs font-bold rounded-lg appearance-none cursor-pointer outline-none transition-all",
                                                            isLocked ? "bg-slate-50 border-slate-100 text-slate-400" : "bg-white border-slate-200 text-slate-700 focus:border-indigo-500 hover:border-slate-300"
                                                        )}
                                                    >
                                                        <option value="-">-</option>
            ...
            <div className="flex flex-wrap items-center gap-6 p-6 bg-slate-50 rounded-[1.5rem] border border-slate-200/60">
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Mapping Scale</span>
                </div>
            ...
            <div className="flex items-center justify-end gap-3 pb-10">
                <button
                    ...
                    disabled={generating || peos.length === 0 || pos.length === 0 || isLocked}
