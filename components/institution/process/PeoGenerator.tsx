...
function getOrderedCriteria(quality: SmartAbetQuality): QualityCriterion[] {
  ...
  return [
    ...
    { key: "abetStyle", label: "ABET Style", passed: quality.abetStyle },
    {
      ...
      passed: Boolean(quality.missionAligned),
    },
  ];
}
...
export default function PeoGenerator({
  ...
  const [generatedPeoQuality, setGeneratedPeoQuality] = useState<
    SmartAbetQuality[]
  >([]);
  ...
  // Process Dates
  const [brainstormingDates, setBrainstormingDates] = useState<DateRange>({
    start: "",
    end: "",
  });
  const [feedbackDates, setFeedbackDates] = useState<DateRange>({
    start: "",
    end: "",
  });
  const [consolidationDates, setConsolidationDates] = useState<DateRange>({
    start: "",
    end: "",
  });

  // UI State
  const [activeTab, setActiveTab] = useState<"config" | "workspace">(
    "workspace",
  );
          ...
          if (programId && instData.programs) {
            ...
            if (currentProgram) {
              ...
              setBrainstormingDates({
                start: currentProgram.peo_brainstorming_start_date
                  ? currentProgram.peo_brainstorming_start_date.split("T")[0]
                  : "",
                end: currentProgram.peo_brainstorming_end_date
                  ? currentProgram.peo_brainstorming_end_date.split("T")[0]
                  : "",
              });
              setFeedbackDates({
                start: currentProgram.peo_feedback_start_date
                  ? currentProgram.peo_feedback_start_date.split("T")[0]
                  : "",
                end: currentProgram.peo_feedback_end_date
                  ? currentProgram.peo_feedback_end_date.split("T")[0]
                  : "",
              });
              setConsolidationDates({
                start: currentProgram.peo_consolidation_start_date
                  ? currentProgram.peo_consolidation_start_date.split("T")[0]
                  : "",
                end: currentProgram.peo_consolidation_end_date
                  ? currentProgram.peo_consolidation_end_date.split("T")[0]
                  : "",
              });
            }
          }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const peoResults = Array.isArray(data.results)
          ? (data.results as string[])
          : [];
        const qualityResults = Array.isArray(data.quality)
          ? (data.quality as SmartAbetQuality[])
          : [];
  ...
  const handleAddPeo = (statement: string, index: number) => {
    setPeos([
      ...
      {
        id: `temp-${Date.now()}`,
        peo_statement: statement,
        peo_number: peos.length + 1,
      },
    ]);
    setGeneratedPeos((previous) =>
      previous.filter((_, currentIndex) => currentIndex !== index),
    );
    setGeneratedPeoQuality((previous) =>
      previous.filter((_, currentIndex) => currentIndex !== index),
    );
  };
  ...
  const handleUpdatePeo = (id: string, newStatement: string) => {
    setPeos(
      peos.map((p) =>
        p.id === id ? { ...p, peo_statement: newStatement } : p,
      ),
    );
  };
  ...
  const handleSaveAll = async () => {
    ...
    dateRanges.forEach(({ name, data }) => {
      if (data.start || data.end) {
        if (!data.start || !data.end) {
          ...
          if (!result.success) {
            isValid = false;
            newErrors[`${name}_dates`] =
              result.error.issues[0]?.message || "Invalid date range";
          }
        }
      }
    });
    ...
    peos.forEach((peo) => {
      ...
      if (!result.success) {
        isValid = false;
        newErrors[`peo_${peo.id}`] =
          result.error.issues[0]?.message || "Invalid PEO";
      }
    });
  ...
  if (!programId)
    return (
      <div className="p-12 text-center text-slate-400 font-light text-lg">
        Please select a program of study to begin.
      </div>
    );
  if (loading)
    return (
    ...
    <div className="space-y-8 pb-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-8">
          ...
          <div
            id="ai-generator"
            className="bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_1px_3px_rgba(0,0,0,0.06)] relative overflow-hidden"
          >
              ...
              {generatedPeos.length > 0 && (
                ...
                >
                  ...
                  {generatedPeos.map((peo, i) => {
                    ...
                    const maxScore =
                      (quality?.maxScore ?? criteria.length) || 7;
                    const percentage =
                      typeof quality?.percentage === "number"
                        ? quality.percentage
                        : Math.round(((quality?.score || 0) / maxScore) * 100);
                    const rating =
                      quality?.rating || resolveQualityRating(percentage);
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-sm text-slate-700 font-medium pr-8">
                          {peo}
                        </p>
                        {quality && (
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div
            id="process-dates"
            className="bg-white rounded-2xl p-6 border border-slate-200 shadow-[0_1px_3px_rgba(0,0,0,0.06)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.04)] transition-shadow"
          >
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className={`w-full h-9 px-3 rounded-lg border text-xs font-medium focus:ring-2 focus:ring-teal-500/20 outline-none text-slate-600 ${errors.brainstorming_dates ? "border-red-500" : "border-slate-200"}`}
                  />
                  <input
                    type="date"
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className={`w-full h-9 px-3 rounded-lg border text-xs font-medium focus:ring-2 focus:ring-teal-500/20 outline-none text-slate-600 ${errors.feedback_dates ? "border-red-500" : "border-slate-200"}`}
                  />
                  <input
                    type="date"
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <input
                    ...
                    className={`w-full h-9 px-3 rounded-lg border text-xs font-medium focus:ring-2 focus:ring-teal-500/20 outline-none text-slate-600 ${errors.consolidation_dates ? "border-red-500" : "border-slate-200"}`}
                  />
                  <input
                    type="date"
              ...
              {peos.length === 0 ? (
                <div className="p-8 text-center border-2 border-dashed border-slate-200 rounded-2xl">
                  <p className="text-slate-400 text-sm">
                    No PEOs defined yet. Use the generator or add one manually.
                  </p>
                </div>
              ) : (
                <AnimatePresence>
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 space-y-3">
                          <textarea
                            ...
                            onChange={(e) =>
                              handleUpdatePeo(peo.id, e.target.value)
                            }
