    ...
    return (
      <div className="flex flex-col justify-center items-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
        ...
        <p className="text-slate-500 font-medium">
          Initialising Consistency Matrix...
        </p>
      </div>
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center rounded-[2.5rem] border-2 border-dashed border-slate-200 bg-slate-50/50">
        ...
        <p className="text-slate-500 max-w-sm mt-2 font-medium">
          Please select a program from the dashboard to generate the Consistency
          Matrix.
        </p>
      </div>
    );
    ...
    <div className="space-y-8 animate-element pb-20">
        ...
        <div className="bg-gradient-to-br from-indigo-50/50 via-white to-indigo-50/30 border border-indigo-100/50 rounded-[2rem] p-8 relative overflow-hidden group shadow-sm">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h4 className="text-[10px] font-black uppercase tracking-[0.25em] text-indigo-600">
            ...
            <thead>
              <tr className="bg-slate-50/80 backdrop-blur-sm border-b border-slate-100">
                ...
                {peos.map((peo, i) => (
                  <th
                    key={peo.id}
                    className="px-6 py-6 text-center min-w-[120px] border-l border-slate-100/50"
                  >
                  ...
                  <td colSpan={peos.length + 1} className="py-20 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-slate-400 font-bold italic">
                        No mission components found to map.
                      </p>
                      <p className="text-xs text-slate-400 mt-1">
                        Please define the program mission in Step 6 first.
                      </p>
                    </div>
                  </td>
                </tr>
                  ...
                  >
                    <td className="px-8 py-5">
                      <div className="flex gap-4">
                        ...
                        <p className="text-sm font-semibold text-slate-700 leading-relaxed pt-1.5">
                          {mission}
                        </p>
                      </div>
                    </td>
                    {peos.map((peo) => (
                      <td
                        key={peo.id}
                        className="px-6 py-5 border-l border-slate-50"
                      >
                            ...
                            <select
                              ...
                              className={cn(
                                ...
                                isLocked
                                  ...
                                  : "bg-white border-slate-200 text-slate-700 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 hover:border-slate-300 active:scale-95",
                              )}
                            >
                              <option value="-">-</option>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-40 bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-[2rem] px-8 py-5 shadow-2xl flex items-center gap-6 ring-1 ring-slate-900/5 min-w-[600px] justify-between">
        <div className="flex items-center gap-4 border-r border-slate-200 pr-6 mr-2">
          <div>
            <h5 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 leading-none mb-1.5">
            ...
            <div className="flex items-center gap-2">
              <div className="h-1.5 w-32 bg-slate-100 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{
                    width: `${Math.min((Object.keys(matrix).filter((k) => matrix[k] !== "-").length / (missionStatements.length * peos.length || 1)) * 100, 100)}%`,
                  }}
              ...
              <span className="text-[10px] font-bold text-indigo-600">
                {Math.round(
                  (Object.keys(matrix).filter((k) => matrix[k] !== "-").length /
                    (missionStatements.length * peos.length || 1)) *
                    100,
                )}
                %
              </span>
        ...
        <div className="flex items-center gap-3">
            ...
            <Tooltip>
              ...
              <TooltipContent className="bg-slate-900 text-white border-transparent">
                <p className="text-xs font-bold">
                  Use AI to suggest correlations based on content
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p className="text-xs font-bold">
                  {isLocked ? "Unlock Matrix" : "Lock Consistency Matrix"}
                </p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
