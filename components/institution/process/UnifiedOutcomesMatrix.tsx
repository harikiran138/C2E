...
const TIER_1_POS = [
    ...
    { code: 'PO6', title: 'Engineer & Society', description: 'Analyze and evaluate societal and environmental aspects while solving complex engineering problems for its impact on sustainability, economy, health, safety, legal framework, culture, and environment.' },
    { code: 'PO7', title: 'Environment & Sustainability', description: 'Understand the impact of professional engineering solutions in societal and environmental contexts, and demonstrate the knowledge of, and need for, sustainable development.' },
    { code: 'PO8', title: 'Ethics', description: 'Apply ethical principles and commit to professional ethics, human values, diversity and inclusion; adhere to national & international laws.' },
    { code: 'PO9', title: 'Individual & Team Work', description: 'Function effectively as an individual, and as a member or leader in diverse teams and multidisciplinary settings, with focus on inclusivity.' },
    { code: 'PO10', title: 'Communication', description: 'Communicate effectively on complex engineering activities with the engineering community and society at large — including comprehension and written reports, presentations, and clear instructions.' },
    { code: 'PO11', title: 'Project Management & Finance', description: 'Demonstrate knowledge and understanding of engineering and management principles and apply these to one’s own work as a member and leader in a team to manage projects in multidisciplinary environments.' },
    { code: 'PO12', title: 'Life-Long Learning', description: 'Recognize the need for, and ability to engage in, independent and lifelong learning in the context of technological change, adaptability and continued relevance.' },
];
...
const TIER_2_POS = [
    ...
    { code: 'PO4', title: 'Conduct Investigations', description: 'Use research-based knowledge and research methods including design of experiments, analysis and interpretation of data, and synthesis of information to provide valid conclusions.' },
    { code: 'PO5', title: 'Engineering Tool Usage', description: 'Create, select, and apply appropriate techniques, resources, and modern engineering and IT tools (including prediction and modeling) to complex engineering activities, with an understanding of the limitations.' },
    { code: 'PO6', title: 'Engineer & Society', description: 'Apply reasoning informed by contextual knowledge to assess societal, health, safety, legal, and cultural issues and the consequent responsibilities relevant to professional engineering practice.' },
    { code: 'PO7', title: 'Environment & Sustainability', description: 'Understand the impact of professional engineering solutions in societal and environmental contexts, and demonstrate the knowledge of, and need for, sustainable development.' },
    ...
    { code: 'PO9', title: 'Individual & Team Work', description: 'Function effectively as an individual, and as a member or leader in diverse teams, and in multidisciplinary settings.' },
    ...
    { code: 'PO11', title: 'Project Management & Finance', description: 'Demonstrate knowledge and understanding of engineering and management principles and apply these to one’s own work, as a member and leader in a team, to manage projects and in multidisciplinary environments.' },
...
function UnifiedOutcomesMatrixContent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (!progResponse.ok) throw new Error(`Details API error: ${progResponse.status}`);

                const data = await progResponse.json();
                const currentProgram = data.programs?.find((p: any) => String(p.id) === String(programId));
                ...
                const poResponse = await fetch(`/api/institution/program-outcomes?programId=${programId}`);
                if (poResponse.ok) {
    ...
    if (!programId) return (
        <div className="flex flex-col items-center justify-center p-20 text-center rounded-[2.5rem] border-2 border-dashed border-slate-200 bg-slate-50/50">
            ...
            <p className="text-slate-500 max-w-sm mt-2 font-medium">Please select a program from the dashboard to continue.</p>
        </div>
    );
                ...
                <section className="space-y-6">
                    <div className="flex items-center justify-between">
                        ...
                        >
                            ...
                            <ChevronRight className={cn("size-4 transition-transform", showMatrix && "rotate-90")} />
                        </button>
                    </div>
                        ...
                        <div className="rounded-[2rem] border border-slate-200 bg-white shadow-xl overflow-hidden">
                                ...
                                <table className="w-full text-sm text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50/80 backdrop-blur-sm border-b border-slate-100">
                                            ...
                                            {peos.map((peo, i) => (
                                                <th key={peo.id || i} className="px-6 py-6 text-center min-w-[100px] border-l border-slate-100/50">
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <div className="cursor-help">
                                                                <div className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em]">PEO {i + 1}</div>
                                                            </div>
                                                        </TooltipTrigger>
                                                        <TooltipContent side="top" className="max-w-xs p-3">
                                                            <p className="text-xs font-semibold">{peo.peo_statement}</p>
                                                        </TooltipContent>
                                                    </Tooltip>
                                                </th>
                                            ))}
                                            ...
                                            <tr key={mIdx} className="hover:bg-slate-50/30 transition-colors">
                                                <td className="px-8 py-5">
                                                    <div className="flex gap-4">
                                                        ...
                                                        <p className="text-sm font-semibold text-slate-700 leading-relaxed">{mission}</p>
                                                    </div>
                                                </td>
                                                {peos.map((peo) => (
                                                    <td key={peo.id} className="px-4 py-5 border-l border-slate-50">
                                                        <div className="flex justify-center">
                                                            <select
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    isLocked ? "bg-slate-50 border-slate-100 text-slate-400" : "bg-white border-slate-200 text-slate-700 focus:border-indigo-500"
                                                                )}
                                                            >
                                                                <option value="-">-</option>
                            ...
                            <div className="p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                                <div className="flex gap-6">
                                    {[
                                        ...
                                        <div key={item.l} className="flex items-center gap-2">
                                            <span className="text-xs font-black text-indigo-600">{item.l}</span>
                                            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{item.t}</span>
                                        </div>
                                    ))}
                                </div>
                                <div className="flex items-center gap-3">
                                    ...
                                    >
                                        {generatingMatrix ? <Loader2 className="size-3.5 animate-spin" /> : <Sparkles className="size-3.5" />}
                                        AI Auto-Fill
                                    </button>
                                    <button
                ...
                <section className="space-y-8 pt-6 border-t border-slate-100">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Generate Program Outcomes</h2>
                                <p className="text-sm text-slate-500 font-medium">Define measurable outcomes for the selected accreditation tier</p>
                            </div>
                        </div>
                        {!showPOs && (
                            <button
                            ...
                            <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-200/60">
                                <div className="flex flex-col sm:flex-row gap-6">
                                    <label className={cn(
                                        ...
                                        tier === 'TIER_1' ? 'bg-indigo-600 border-indigo-600 text-white ring-4 ring-indigo-50' : 'bg-white border-slate-200 hover:border-indigo-300'
                                    )}>
                                        <input
                                            type="radio"
                                            name="tier"
                                            ...
                                            className="hidden"
                                        />
                                        <div className={cn("size-6 rounded-full border-2 flex items-center justify-center", tier === 'TIER_1' ? "border-white" : "border-slate-300")}>
                                            {tier === 'TIER_1' && <div className="size-3 rounded-full bg-white" />}
                                        </div>
                                        <div>
                                            <span className={cn("block font-bold text-base", tier === 'TIER_1' ? "text-white" : "text-slate-900")}>Tier I</span>
                                            <span className={cn("text-xs font-semibold opacity-80", tier === 'TIER_1' ? "text-indigo-50" : "text-slate-500")}>Include SDGs and Washington Accord (WK)</span>
                                        </div>
                                    </label>

                                    <label className={cn(
                                        "flex items-center gap-4 p-5 rounded-2xl border cursor-pointer transition-all flex-1 shadow-sm",
                                        tier === 'TIER_2' ? 'bg-indigo-600 border-indigo-600 text-white ring-4 ring-indigo-50' : 'bg-white border-slate-200 hover:border-indigo-300'
                                    )}>
                                        <input
                                            type="radio"
                                            name="tier"
                                            ...
                                            className="hidden"
                                        />
                                        <div className={cn("size-6 rounded-full border-2 flex items-center justify-center", tier === 'TIER_2' ? "border-white" : "border-slate-300")}>
                                            {tier === 'TIER_2' && <div className="size-3 rounded-full bg-white" />}
                                        </div>
                                        <div>
                                            <span className={cn("block font-bold text-base", tier === 'TIER_2' ? "text-white" : "text-slate-900")}>Tier II</span>
                                            <span className={cn("text-xs font-semibold opacity-80", tier === 'TIER_2' ? "text-indigo-50" : "text-slate-500")}>Standard Program Outcomes</span>
                                        </div>
                                    </label>
                                </div>
                            </div>
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex items-center justify-between">
                                            <h3 className="text-2xl font-black text-slate-900 tracking-tight">
                                                Program Outcomes <span className="text-indigo-600">PO1 – PO12</span>
                                            </h3>
                                            <button
                                                ...
                                                >
                                                    <div className="flex gap-6">
                                                        ...
                                                        <div className="space-y-2">
                                                            <h4 className="font-extrabold text-slate-900 text-base tracking-tight">{po.title || po.po_title}</h4>
                ...
                <section className="space-y-8 pt-6 border-t border-slate-100">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">PEO to PO Mapping Matrix</h2>
                                <p className="text-sm text-slate-500 font-medium">Map Program Educational Objectives to Outcomes</p>
                            </div>
                        </div>
                        <button
                        ...
                        >
                            ...
                            <ChevronRight className={cn("size-4 transition-transform", showPeoPoMapping && "rotate-90")} />
                        </button>
                    </div>
