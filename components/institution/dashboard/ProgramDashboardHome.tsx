...
export default function ProgramDashboardHome({
  statsData,
  loading,
  selectedProgramId,
}: ProgramDashboardHomeProps) {
  // Pinned module keys - loading from DB preferences if available
  const [pinnedKeys, setPinnedKeys] = useState<string[]>(
    ensureCoreModules(["process-4", "process-3", "process-5"]),
  );
  ...
  React.useEffect(() => {
    if (selectedProgramId) {
      console.log(
        "[Dashboard] Loading preferences for program:",
        selectedProgramId,
      );
      fetch(
        `/api/institution/dashboard/preferences?programId=${selectedProgramId}`,
      )
        .then((res) => res.json())
        .then((data) => {
        ...
        {pinnedKeys.map((key) => {
          ...
          return (
            <motion.div key={key} variants={item}>
              <Link
                href={`/institution/process/${key}?programId=${selectedProgramId}`}
              >
                <Card className="h-full border border-slate-200 bg-white hover:border-indigo-300 transition-all duration-300 shadow-sm hover:shadow-md flex flex-col justify-between p-4 rounded-lg group relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <h3 className="font-bold text-slate-900 text-[13px] mb-0.5 group-hover:text-indigo-600 transition-colors line-clamp-1">
                      {step.title || "Untitled"}
                    </h3>
        ...
        <motion.div variants={item}>
          ...
          >
            ...
            <div className="text-center">
              <span className="block text-[10px] font-bold text-slate-400 group-hover:text-indigo-600 uppercase tracking-widest">
                Pin More
              </span>
            </div>
          </button>
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                    Select modules to display on home
                  </p>
                </div>
                <button
                  onClick={() => setShowCustomizePanel(false)}
