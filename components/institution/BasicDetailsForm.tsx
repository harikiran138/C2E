...
export default function BasicDetailsForm() {
  ...
  const fetchDetails = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/institution/details', { cache: 'no-store' });
      if (res.status === 401) {
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-200 dark:border-slate-800 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label className="block text-sm font-semibold text-slate-500 mb-2">Institution Name</label>
                    <input 
                        value={instName}
                 ...
                 <div>
                    <label className="block text-sm font-semibold text-slate-500 mb-2">Institution Type</label>
                    <select
                        value={instType}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label className="block text-sm font-semibold text-slate-500 mb-2">Established Year</label>
                    <input 
                        type="number"
                        value={estYear}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                     <label className="block text-sm font-semibold text-slate-500 mb-2">State</label>
                     <input 
                         value={state}
            ...
            <div>
                 <label className="block text-sm font-semibold text-slate-500 mb-2">University Affiliation</label>
                 <input 
                     value={affiliation}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-200 dark:border-slate-800">
             ...
             {isAddingProgram && (
                <div className="mb-6 p-6 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                     ...
                     <div>
                        <label className="text-xs font-bold text-slate-500 mb-1 block">Program Code</label>
                        <input 
                     ...
                     <div>
                         <label className="text-xs font-bold text-slate-500 mb-1 block">Duration (Years)</label>
                         <input 
                           type="number"
                     ...
                     <div>
                         <label className="text-xs font-bold text-slate-500 mb-1 block">Intake</label>
                         <input 
                           type="number"
                     ...
                     <div>
                         <label className="text-xs font-bold text-slate-500 mb-1 block">Est. Year</label>
                         <input 
                     ...
                     <button onClick={handleAddProgram} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-6 rounded-lg transition-colors shadow-lg shadow-green-600/20">
                        Add Program
                     </button>
                </div>
             )}
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-slate-100 dark:border-slate-800">
                            ...
                            <th className="py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Code</th>
                            ...
                            <th className="py-4 text-xs font-bold text-slate-500 uppercase tracking-wider">Intake</th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {programs.map((prog) => (
                            <tr key={prog.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                <td className="py-4 font-semibold text-slate-700 dark:text-slate-200">{prog.program_name}</td>
                                <td className="py-4 text-slate-600 dark:text-slate-400 font-mono text-xs">{prog.program_code}</td>
                                <td className="py-4 text-slate-600 dark:text-slate-400">{prog.degree}</td>
                                <td className="py-4 text-slate-600 dark:text-slate-400">{prog.duration} Years</td>
                                <td className="py-4 text-slate-600 dark:text-slate-400">{prog.intake}</td>
                                <td className="py-4">
