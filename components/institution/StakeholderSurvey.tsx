  ...
  if (submitted) {
    return (
      <div className="flex items-center justify-center p-4 py-20">
        <div className="bg-white dark:bg-slate-800 p-12 rounded-xl shadow-2xl max-w-lg w-full text-center space-y-6 border border-slate-100 dark:border-slate-700 animate-in zoom-in-95 duration-500">
          ...
          <div className="space-y-2">
            <h2 className="text-3xl font-semibold text-slate-900 dark:text-white font-serif">Thank You!</h2>
            <p className="text-slate-500 dark:text-slate-400 font-medium">Your feedback is invaluable in shaping the future of our <span className="text-indigo-600 font-semibold">{programName}</span> program.</p>
          </div>
          <div className="flex flex-col gap-3">
            <button
              onClick={() => setSubmitted(false)}
              className="px-8 py-3 bg-white border border-slate-200 text-slate-700 font-semibold rounded-lg hover:bg-slate-50 transition-all shadow-sm"
            >
              Update Response
            </button>
            {onBack && (
              <button
                onClick={onBack}
                className="px-8 py-3 bg-slate-900 dark:bg-slate-700 text-white font-semibold rounded-lg hover:scale-105 active:scale-95 transition-all shadow-lg"
              >
                Return to Dashboard
              </button>
            )}
  ...
  const SectionHeader = ({ icon: Icon, title, id }: { icon: any, title: string, id: string }) => (
    <div id={id} className="pt-16 mb-6 first:pt-0 scroll-mt-24">
      <div className="flex items-center gap-3 mb-4">
        ...
        <h3 className="text-xl font-semibold text-slate-900 dark:text-white font-serif">{title}</h3>
      </div>
    ...
    <div className="bg-slate-50 dark:bg-slate-900 min-h-screen font-sans py-12 px-4 selection:bg-primary-gold/30">
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-8 items-start">
        ...
        <aside className="hidden lg:block w-72 sticky top-12 space-y-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-xl border border-slate-100 dark:border-slate-700">
            <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-[0.2em] mb-6 px-2">Navigation</h4>
            <div className="space-y-1">
              {sections.map((s) => (
                <a
                  key={s.id}
                  href={`#${s.id}`}
                  className="flex items-center gap-3 p-3 rounded-xl transition-all hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-600 dark:text-slate-400 font-semibold text-sm group"
                >
          ...
          <div className="bg-slate-900 rounded-xl p-6 text-white shadow-xl relative overflow-hidden">
            ...
            <h5 className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-4">Your Progress</h5>
            <div className="space-y-2">
              ...
              <p className="text-[10px] font-semibold text-primary-gold uppercase tracking-widest">{Math.round(calculateProgress())}% Completed</p>
            </div>
          </div>
        </aside>

        <div className="flex-1 max-w-4xl bg-white dark:bg-slate-800 rounded-xl shadow-2xl overflow-hidden border border-slate-100 dark:border-slate-700">
          {/* Header */}
          <div className="bg-slate-900 p-12 text-white relative overflow-hidden">
            ...
            <div className="relative z-10 space-y-4">
              {onBack && (
                <button
                  onClick={onBack}
                  className="inline-flex items-center gap-2 mb-2 text-primary-gold hover:text-white transition-colors text-sm font-semibold"
                >
                  <ArrowLeft className="size-4" /> Back to Dashboard
                </button>
              )}
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full border border-white/20 text-xs font-semibold uppercase tracking-widest text-primary-gold">
                <ClipboardCheck className="size-3" /> Stakeholder Consultation
              </div>
              <h1 className="text-4xl font-semibold font-serif leading-tight">Program Educational Objectives (PEOs) – Stakeholder Survey</h1>
          ...
          <form onSubmit={handleSubmit} className="p-12 space-y-12">
            ...
            <section>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest ml-1">Stakeholder Category *</label>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest ml-1">Organization / Institution</label>
                  <input
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest ml-1">Designation / Relationship</label>
                  <input
                ...
                <div className="md:col-span-2 space-y-3">
                  <label className="text-xs font-semibold text-slate-400 uppercase tracking-widest ml-1">Years of association with Mechanical Engineering</label>
                  <div className="flex flex-wrap gap-4">
                    {['< 3 years', '3–5 years', '5–10 years', '>10 years'].map(opt => (
                      <label key={opt} className={`flex-1 min-w-[120px] p-4 rounded-lg border transition-all cursor-pointer flex items-center justify-center gap-2 font-semibold text-sm ${formData.association_years === opt
            ...
            <section>
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Professional roles a graduate should be able to perform 3–5 years after graduation:</p>
                  <div className="grid sm:grid-cols-2 gap-3">
                    {['Design / Analysis Engineer', 'Manufacturing / Production Engineer', 'R&D / Research Engineer', 'Maintenance / Quality Engineer', 'Higher Studies / Research Scholar', 'Entrepreneur / Start-up Founder', 'Managerial / Leadership Roles'].map(role => (
                      <label key={role} className={`p-4 rounded-lg border transition-all cursor-pointer flex items-center gap-3 font-medium text-sm ${formData.career_roles.includes(role)
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">What defines professional success for a graduate?</p>
                  <div className="flex flex-wrap gap-3">
                    {['Career growth', 'Job stability', 'Leadership responsibilities', 'Research / innovation contributions', 'Societal impact'].map(item => (
                      <label key={item} className={`px-5 py-3 rounded-full border transition-all cursor-pointer font-semibold text-xs uppercase tracking-wider ${formData.success_definition.includes(item)
            ...
            <section>
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Rate the importance of strong fundamentals for long-term success:</p>
                  <div className="flex gap-4">
                    {['Very High', 'High', 'Moderate', 'Low'].map(opt => (
                      <label key={opt} className={`flex-1 p-4 rounded-lg border text-center transition-all cursor-pointer font-semibold text-sm ${formData.fundamentals_importance === opt
                          ? 'bg-slate-900 text-white border-slate-900 shadow-lg'
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Critical core domains for future careers (Multiple choice):</p>
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {['Engineering Mechanics', 'Thermal Engineering', 'Fluid Mechanics', 'Manufacturing Processes', 'Materials & Metallurgy', 'Machine Design', 'Mechatronics / Automation'].map(domain => (
                      <label key={domain} className={`p-4 rounded-lg border transition-all cursor-pointer flex items-center gap-3 font-medium text-sm ${formData.core_domains.includes(domain)
                          ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Problem-solving ability level expected after 3–5 years:</p>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {['Routine problem execution', 'Analytical problem solving', 'System-level thinking', 'Innovation-oriented problem solving'].map(level => (
                      <label key={level} className={`p-5 rounded-[1.5rem] border transition-all cursor-pointer space-y-2 ${formData.problem_solving_level === level
                        ...
                        <div className="flex items-center justify-between">
                          <span className={`text-sm font-semibold ${formData.problem_solving_level === level ? 'text-slate-900' : 'text-slate-600'}`}>{level}</span>
            ...
            <section>
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {['CAD / CAE / CFD / FEA', 'Additive Manufacturing', 'Robotics & Automation', 'Digital Manufacturing / Industry 4.0', 'AI / ML applications', 'Sustainable & Green Technologies'].map(area => (
                      <label key={area} className={`p-4 rounded-lg border transition-all cursor-pointer flex items-center gap-3 font-medium text-sm ${formData.emerging_areas.includes(area)
                          ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Importance of multidisciplinary capability (Mechanical + Electrical + Software):</p>
                  <div className="flex gap-4">
                    {['Very High', 'High', 'Moderate', 'Low'].map(opt => (
                      <label key={opt} className={`flex-1 p-4 rounded-lg border text-center transition-all cursor-pointer font-semibold text-sm ${formData.multidisciplinary_importance === opt
                          ? 'bg-slate-900 text-white border-slate-900 shadow-lg'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
            ...
            <section>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Importance of ethics, safety, and quality practices:</p>
                  <div className="flex gap-4">
                    {['Very High', 'High', 'Moderate', 'Low'].map(opt => (
                      <label key={opt} className={`flex-1 p-4 rounded-lg border text-center transition-all cursor-pointer font-semibold text-sm ${formData.ethics_importance === opt
                          ? 'bg-slate-900 text-white border-slate-900 shadow-lg'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                        <input
            ...
            <section>
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Should graduates be encouraged towards continuous learning?</p>
                  <div className="flex gap-4">
                    {['Yes', 'No', 'Maybe'].map(opt => (
                      <label key={opt} className={`flex-1 p-4 rounded-lg border text-center transition-all cursor-pointer font-semibold text-sm ${formData.lifelong_learning === opt
                          ? 'bg-slate-900 text-white border-slate-900 shadow-lg'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Pathways PEOs should explicitly support (Multiple choice):</p>
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {['Higher Studies (M.Tech / MS / Ph.D.)', 'Professional Certifications', 'Research & Innovation', 'Entrepreneurship', 'Professional Body Memberships'].map(path => (
                      <label key={path} className={`p-4 rounded-lg border transition-all cursor-pointer flex items-center gap-3 font-medium text-sm ${formData.pathways.includes(path)
                          ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Importance of exposure to research, innovation, and patents:</p>
                  <div className="flex gap-4">
                    {['Very High', 'High', 'Moderate', 'Low'].map(opt => (
                      <label key={opt} className={`flex-1 p-4 rounded-lg border text-center transition-all cursor-pointer font-semibold text-sm ${formData.research_importance === opt
                          ? 'bg-slate-900 text-white border-slate-900 shadow-lg'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
            ...
            <section>
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Should graduates be prepared for global careers?</p>
                  <div className="flex gap-4">
                    {['Yes', 'No', 'Depends'].map(opt => (
                      <label key={opt} className={`flex-1 p-4 rounded-lg border text-center transition-all cursor-pointer font-semibold text-sm ${formData.global_career === opt
                          ? 'bg-slate-900 text-white border-slate-900 shadow-lg'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Role Mechanical engineers should play in societal/national development:</p>
                  <div className="grid sm:grid-cols-2 gap-3">
                    {['Sustainable engineering', 'Energy efficiency & environment', 'Infrastructure & manufacturing growth', 'Community welfare'].map(role => (
                      <label key={role} className={`p-4 rounded-lg border transition-all cursor-pointer flex items-center gap-3 font-medium text-sm ${formData.societal_role.includes(role)
                          ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-400'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
            ...
            <section>
              ...
              <div className="space-y-8">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">What should distinguish graduates of this Mechanical Engineering program from others?</label>
                  <textarea
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
                  <div className="overflow-x-auto">
                    <table className="w-full text-left min-w-[500px]">
                      <thead className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                          <th className="p-5 text-sm font-semibold text-slate-400 uppercase tracking-widest">Aspect Emphasis</th>
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {['Academic excellence', 'Skill development', 'Values & ethics', 'Innovation & research'].map(aspect => (
                          <tr key={aspect} className="hover:bg-slate-100/50 dark:hover:bg-slate-800/30 transition-colors">
                            <td className="p-5 text-sm font-semibold text-slate-700 dark:text-slate-300">{aspect}</td>
            ...
            <section>
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">In your view, 3–5 years after graduation, a graduate should demonstrate:</p>
                  <div className="grid gap-3">
                    {[
                      ...
                      <label key={item} className={`p-5 rounded-lg border transition-all cursor-pointer flex items-center gap-4 ${formData.peo_validation.includes(item)
                          ? 'bg-green-50 dark:bg-green-900/10 border-green-200 dark:border-green-800 text-green-800 dark:text-green-400 font-semibold'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                        <div className={`size-6 rounded-full border-2 flex items-center justify-center transition-all ${formData.peo_validation.includes(item) ? 'bg-green-500 border-transparent' : 'bg-white border-slate-200'
                          }`}>
                ...
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Do you feel stakeholder expectations are adequately reflected in current PEOs?</p>
                  <div className="flex gap-4">
                    {['Yes', 'Partially', 'No'].map(opt => (
                      <label key={opt} className={`flex-1 p-4 rounded-lg border text-center transition-all cursor-pointer font-semibold text-sm ${formData.expectations_reflected === opt
                          ? 'bg-slate-900 text-white border-slate-900 shadow-lg'
                          : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400'
                        }`}>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">What future competencies should PEOs emphasize more strongly?</label>
                  <textarea
            ...
            <section>
              ...
              <div className="space-y-4">
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Any additional comments or recommendations for improving PEOs:</p>
                <textarea
                  placeholder="Your suggestions here..."
                  rows={4}
                  value={formData.additional_comments}
                  onChange={e => setFormData({ ...formData, additional_comments: e.target.value })}
                  className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-6 rounded-xl outline-none focus:border-primary-gold focus:ring-4 focus:ring-primary-gold/5 transition-all font-medium text-slate-800 dark:text-slate-200 shadow-inner"
                />
              </div>
            </section>
            ...
            <div className="pt-12 border-t border-slate-100 dark:border-slate-700">
              ...
              <p className="text-center text-[10px] text-slate-400 font-semibold uppercase tracking-[0.2em] mt-6 flex items-center justify-center gap-2">
