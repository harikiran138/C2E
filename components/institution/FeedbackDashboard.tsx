      ...
      setResponses([
        {
          ...
          category: "Alumni",
          rating: 5,
          comment: "Great vision for the program.",
        ...
        },
        {
          ...
          category: "Employer",
          rating: 4,
          comment: "Needs more focus on practical skills.",
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400">
              Analyze stakeholder feedback and manage surveys.
            </p>
          </div>
          <button
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col justify-between">
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <button
                ...
                className="text-[#137fec] text-sm font-bold hover:bg-blue-50 dark:hover:bg-blue-900/20 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
              >
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-200 dark:border-slate-800">
          ...
          <div className="space-y-4">
            {responses.map((res) => (
              <div
                key={res.id}
                className="p-6 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 transition-all hover:shadow-md"
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex gap-3 items-center">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white">
                        {res.stakeholder_name}
                      </h4>
                      <span className="text-xs font-medium px-2 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                        {res.category}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center text-yellow-500">
                    {[...Array(5)].map((_, i) => (
                      <span
                        key={i}
                        className={`material-symbols-outlined text-[18px] ${i < res.rating ? "fill-current" : "text-slate-300"}`}
                      >
                        star
                      </span>
                    ))}
