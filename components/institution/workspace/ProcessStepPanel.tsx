...
const CATEGORY_CELL_FIELDS = [
  'design_percent', 'courses_t', 'courses_p', 'courses_tu', 'courses_ll',
  'hours_ci', 'hours_t', 'hours_li', 'hours_twd', 'hours_total', 'credit'
];
...
const CURRICULUM_STRUCTURE_ROWS = [
  ...
  { category: 'Open Electives (Inter-departmental)', code: 'OE', min: 10, max: 15 },
...
const CONVENTIONAL_ELECTIVE_OPTIONS = ['None', 'BS', 'ES', 'HSS', 'PC', 'OE', 'MC', 'AE', 'SE'];
...
const SEMESTER_CATEGORY_COLUMNS = ['BS', 'ES', 'HSS', 'PC', 'OE', 'MC', 'AE', 'SE', 'INT', 'PRO', 'OTHERS'];
...
function CurriculumStructurePanel() {
  ...
  const [transDisciplinaryElective, setTransDisciplinaryElective] = useState('None');
  ...
  useEffect(() => {
    ...
    setCategoryCredits(CURRICULUM_STRUCTURE_ROWS.map(r => ({
      category_code: r.code,
      design_percent: 0,
      courses_t: 0, courses_p: 0, courses_tu: 0, courses_ll: 0,
      hours_ci: 0, hours_t: 0, hours_li: 0, hours_twd: 0, hours_total: 0,
      credit: 0
    })));

    setSemesterCategories(SEMESTERS.map(s => ({
      semester: s,
      no_of_credits: 0,
      courses_bs: 0, courses_es: 0, courses_hss: 0, courses_pc: 0,
      courses_oe: 0, courses_mc: 0, courses_ae: 0, courses_se: 0,
      courses_int: 0, courses_pro: 0, courses_others: 0
    })));
        ...
        .then(data => {
          if (data.categoryCredits?.length > 0) {
            setCategoryCredits((prev) =>
              prev.map(row => {
                const found = data.categoryCredits.find((c: any) => c.category_code === row.category_code);
                return found ? { ...row, ...found } : row;
              })
            );
          }
          if (data.electivesSettings) {
            setConventionalElective(data.electivesSettings.conventional_elective || 'None');
            setTransDisciplinaryElective(data.electivesSettings.trans_disciplinary_elective || 'None');
          ...
          if (data.semesterCategories?.length > 0) {
            setSemesterCategories((prev) =>
              prev.map(row => {
                const found = data.semesterCategories.find((s: any) => s.semester === row.semester);
                return found ? { ...row, ...found } : row;
              })
            );
          }
  ...
  const handleCategoryCreditChange = (code: string, field: string, value: string) => {
    setCategoryCredits(prev => prev.map(c => c.category_code === code ? { ...c, [field]: Number(value) } : c));
  };

  const handleSemesterCategoryChange = (semester: string, field: string, value: string) => {
    setSemesterCategories(prev => prev.map(s => s.semester === semester ? { ...s, [field]: Number(value) } : s));
  };
  ...
  const saveCurriculum = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`/api/curriculum/structure?programId=${programId}`, {
        method: 'POST',
    ...
    <div className="space-y-8">
      <div className="space-y-2 flex justify-between items-center pr-2">
        <div>
          ...
          <p className="text-sm text-slate-600">Curriculum Structure, Learning Hours & Credits</p>
        </div>
        <button
        ...
        <table className="w-full min-w-[1300px] border-collapse text-xs">
          <thead>
            <tr className="bg-slate-50 text-slate-700">
              <th rowSpan={2} className="border-b border-r border-slate-200 px-3 py-3 text-left font-semibold">Category</th>
              <th rowSpan={2} className="border-b border-r border-slate-200 px-3 py-3 text-left font-semibold">Code</th>
              <th colSpan={2} className="border-b border-r border-slate-200 px-3 py-3 text-center font-semibold">Typical Credit Range (%)</th>
              <th rowSpan={2} className="border-b border-r border-slate-200 px-3 py-3 text-center font-semibold">Your Design %</th>
              <th colSpan={4} className="border-b border-r border-slate-200 px-3 py-3 text-center font-semibold">Number of Courses</th>
              <th colSpan={6} className="border-b border-slate-200 px-3 py-3 text-center font-semibold">Suggested Learning Hours & Credit</th>
            </tr>
            <tr className="bg-slate-50 text-slate-600">
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">Min</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">Max</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">T</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">P</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">TU</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">LL</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">CI</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">T</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">LI</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">TW/D</th>
              <th className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">Total</th>
              <th className="border-b border-slate-200 px-2 py-2 text-center font-semibold">Credit</th>
            </tr>
          </thead>
          <tbody>
            {CURRICULUM_STRUCTURE_ROWS.map((row) => {
              const rowData = categoryCredits.find(c => c.category_code === row.code) || {};
              return (
                <tr key={row.code} className="hover:bg-slate-50/50">
                  <td className="border-b border-r border-slate-100 px-3 py-2 font-medium text-slate-700">{row.category}</td>
                  <td className="border-b border-r border-slate-100 px-3 py-2 text-slate-600">{row.code}</td>
                  <td className="border-b border-r border-slate-100 px-3 py-2 text-center text-slate-600">{row.min}</td>
                  <td className="border-b border-r border-slate-100 px-3 py-2 text-center text-slate-600">{row.max}</td>
                  {CATEGORY_CELL_FIELDS.map((field, cellIndex) => (
                    <td key={`${row.code}-cell-${cellIndex}`} className="border-b border-r border-slate-100 px-2 py-1.5">
                      <input
                        type="number"
                        min={0}
                        step={0.1}
                        value={rowData[field] || ''}
                        onChange={(e) => handleCategoryCreditChange(row.code, field, e.target.value)}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(event) => setTransDisciplinaryElective(event.target.value)}
            className="h-11 w-full rounded-xl border border-slate-300 px-3 text-sm focus:border-slate-500 focus:outline-none"
          >
      ...
      <div className="space-y-2">
        <h4 className="text-base font-semibold text-slate-900">Select the Semester & Category of Courses</h4>
        <div className="max-w-xs">
          ...
          />
          {totalCreditsError && <p className="mt-1 text-xs font-medium text-red-600">{totalCreditsError}</p>}
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white">
        <table className="w-full min-w-[1100px] border-collapse text-xs">
          <thead>
            <tr className="bg-slate-50 text-slate-700">
              <th rowSpan={2} className="border-b border-r border-slate-200 px-3 py-3 text-left font-semibold">Semester</th>
              <th rowSpan={2} className="border-b border-r border-slate-200 px-3 py-3 text-left font-semibold">No. of Credits</th>
              <th colSpan={SEMESTER_CATEGORY_COLUMNS.length} className="border-b border-slate-200 px-3 py-3 text-center font-semibold">Number of Courses</th>
            </tr>
            <tr className="bg-slate-50 text-slate-600">
              {SEMESTER_CATEGORY_COLUMNS.map((column) => (
                <th key={column} className="border-b border-r border-slate-200 px-2 py-2 text-center font-semibold">
                  {column}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {SEMESTERS.map((semester) => {
              const semData = semesterCategories.find(s => s.semester === semester) || {};
              return (
                <tr key={semester} className="hover:bg-slate-50/50">
                  <td className="border-b border-r border-slate-100 px-3 py-2 font-semibold text-slate-700">{semester}</td>
                  <td className="border-b border-r border-slate-100 px-2 py-1.5">
                    <input
                      ...
                      onChange={(e) => handleSemesterCategoryChange(semester, 'no_of_credits', e.target.value)}
                      className="w-full rounded-md border border-slate-200 px-2 py-1 text-right text-xs focus:border-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                      placeholder="0"
                    />
                  </td>
                  {SEMESTER_CATEGORY_COLUMNS.map((column) => {
                    ...
                    return (
                      <td key={`${semester}-${column}`} className="border-b border-r border-slate-100 px-2 py-1.5">
                        <input
                          type="number"
                          min={0}
                          step={1}
                          value={semData[fieldName] || ''}
                          onChange={(e) => handleSemesterCategoryChange(semester, fieldName, e.target.value)}
                          className="w-full rounded-md border border-slate-200 px-2 py-1 text-right text-xs focus:border-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                          placeholder="0"
                        />
                      </td>
