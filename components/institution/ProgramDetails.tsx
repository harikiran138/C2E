        ...
        <>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-200 dark:border-slate-800 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div>
                        <label className="block text-sm font-semibold text-slate-500 mb-2">Program Chair</label>
                        <input 
                     ...
                     <div>
                        <label className="block text-sm font-semibold text-slate-500 mb-2">NBA Coordinator</label>
                         <input 
                ...
                <div>
                    <label className="block text-sm font-semibold text-slate-500 mb-2">Vision</label>
                    <textarea 
                 ...
                 <div>
                    <label className="block text-sm font-semibold text-slate-500 mb-2">Mission</label>
                    <textarea 
                ...
                <button onClick={handleProgramUpdate} className="bg-[#137fec] text-white font-bold py-3 px-6 rounded-xl shadow-lg shadow-blue-500/20 hover:bg-[#137fec]/90 transition-all">
                    Save Changes
                </button>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-200 dark:border-slate-800">
                ...
                {isAddingStakeholder && (
                    <div className="mb-6 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl space-y-4 border border-slate-200 dark:border-slate-700">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         ...
                         <button onClick={handleAddStakeholder} className="w-full bg-[#137fec] text-white font-bold py-3 rounded-lg shadow-md hover:bg-[#137fec]/90">
                ...
                <div className="space-y-4">
                    {stakeholders.length === 0 ? (
                        ...
                        stakeholders.map((sh) => (
                             <div key={sh.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-100 dark:border-slate-800 gap-4">
                                <div>
                                    <h4 className="font-bold text-slate-800 dark:text-slate-200">{sh.name}</h4>
                                    <p className="text-xs text-slate-500 uppercase tracking-wider">{sh.category} • {sh.organization}</p>
                                    <p className="text-xs text-slate-400 mt-1">{sh.email} • {sh.contact}</p>
                                </div>
                                <div className="flex items-center gap-3 w-full md:w-auto">
                                   <span className="px-2 py-1 bg-white dark:bg-slate-900 rounded text-xs font-semibold text-green-600 border border-green-200">Survey Active</span>
                                   <button 
                                     onClick={() => sh.id && handleDeleteStakeholder(sh.id)}
