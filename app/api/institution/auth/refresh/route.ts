...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const isMatch = await bcrypt.compare(refreshToken, institution.refresh_token_hash);
      if (!isMatch) {
      ...
      await client.query('UPDATE public.institutions SET refresh_token_hash = $1 WHERE id = $2', [newHash, institutionId]);
