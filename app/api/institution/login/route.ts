...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const invalidCredsResponse = () => NextResponse.json({ error: 'Invalid credentials.' }, { status: 401 });
      ...
      if (institution.locked_until && new Date() < new Date(institution.locked_until)) {
        await logAudit({
        ...
        return NextResponse.json({ error: 'Account is locked due to too many failed attempts. Try again in 15 minutes.' }, { status: 403 });
      }
      ...
      if (!match) {
        ...
        if (newFailedAttempts >= 5) {
            ...
            await client.query('UPDATE public.institutions SET failed_attempts = $1, locked_until = $2 WHERE id = $3', [newFailedAttempts, lockTime, institution.id]);
        } else {
            await client.query('UPDATE public.institutions SET failed_attempts = $1 WHERE id = $2', [newFailedAttempts, institution.id]);
        }
      ...
      if (institution.failed_attempts > 0 || institution.locked_until) {
        await client.query('UPDATE public.institutions SET failed_attempts = 0, locked_until = NULL WHERE id = $1', [institution.id]);
      }
      ...
      await client.query('UPDATE public.institutions SET refresh_token_hash = $1 WHERE id = $2', [refreshTokenHash, institution.id]);
