...
export async function GET(request: Request) {
  try {
    ...
    try {
      const params: any[] = [];
      const where: string[] = [];
      let query = 'SELECT pm.* FROM pac_members pm';

      if (context.mode === 'institution') {
        query += ' INNER JOIN programs p ON p.id = pm.program_id';
      ...
      if (requestedProgramId) {
        ...
        where.push(`pm.program_id = $${params.length}`);
      ...
      if (memberId) {
        ...
        where.push(`pm.id = $${params.length}`);
      }
...
export async function PUT(request: Request) {
  try {
    ...
    const client = await pool.connect();
    try {
      const currentRecord = await client.query('SELECT program_id FROM pac_members WHERE id = $1 LIMIT 1', [id]);
  ...
  try {
    ...
    try {
      if (context.mode === 'program') {
        await client.query('DELETE FROM pac_members WHERE id = $1 AND program_id = $2', [id, context.programId]);
      } else {
        await client.query(
          `DELETE FROM pac_members
           WHERE id = $1
