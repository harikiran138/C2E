...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      await client.query('DELETE FROM program_outcomes WHERE program_id = $1', [program_id]);
      ...
      for (const po of pos) {
          await client.query(
             `INSERT INTO program_outcomes (program_id, po_code, po_title, po_description, tier)
              VALUES ($1, $2, $3, $4, $5)`,
             [program_id, po.code || po.po_code, po.title || po.po_title, po.description || po.po_description, tier]
          );
      }
