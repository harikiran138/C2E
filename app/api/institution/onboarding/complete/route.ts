...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: details, error: instError } = await supabase
      .from('institutions')
      .select('id, institution_type, institution_status, established_year, university_affiliation, address, city, state, email')
      .eq('id', institutionId)
      .single();
    
    if (instError || !details) {
    ...
    if (progError) {
      console.error('Error checking programs for completion:', progError);
    }

    if (!programCount || programCount < 1) {
