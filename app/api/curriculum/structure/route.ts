...
export async function GET(request: Request) {
  try {
    ...
    const [
      categoryCreditsResponse,
      electivesSettingsResponse,
      semesterCategoriesResponse,
    ] = await Promise.all([
      supabase
    ...
    if (semesterCategoriesResponse.error)
      throw semesterCategoriesResponse.error;
    // electivesSettingsResponse might return PGRST116 (No rows found) if it's the first time
    if (
      electivesSettingsResponse.error &&
      electivesSettingsResponse.error.code !== "PGRST116"
    ) {
  ...
  try {
    ...
    if (categoryCredits && Array.isArray(categoryCredits)) {
      ...
      const { error } = await supabase
        .from("curriculum_category_credits")
        .upsert(formattedCategoryCredits, {
    ...
    if (semesterCategories && Array.isArray(semesterCategories)) {
      const formattedSemesterCategories = semesterCategories.map(
        (item: any) => ({
          ...item,
          program_id: programId,
        }),
      ...
      const { error } = await supabase
        .from("curriculum_semester_categories")
        .upsert(formattedSemesterCategories, {
