...
async function getStakeholderPayload(request: NextRequest): Promise<StakeholderTokenPayload | null> {
  ...
  if (!payload.stakeholder_ref_id || !payload.stakeholder_member_id || !payload.program_id) return null;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const peos = Array.isArray(body?.peos) ? (body.peos as PEOFeedbackInput[]) : [];

    if (!isValidRating(vision.rating) || !isValidRating(mission.rating)) {
      return NextResponse.json({ error: 'Vision and Mission ratings must be between 1 and 5.' }, { status: 400 });
    }
    ...
    try {
      ...
      if (!stakeholderRow.is_approved) {
        await client.query('ROLLBACK');
        return NextResponse.json({ error: 'Stakeholder is not approved for feedback.' }, { status: 403 });
      }

      const startAt = stakeholderRow.vmpeo_feedback_start_at ? new Date(stakeholderRow.vmpeo_feedback_start_at) : null;
      const endAt = stakeholderRow.vmpeo_feedback_end_at ? new Date(stakeholderRow.vmpeo_feedback_end_at) : null;
      const now = new Date();
      if (!startAt || !endAt || now < startAt || now > endAt) {
        await client.query('ROLLBACK');
        return NextResponse.json({ error: 'Feedback submission is outside the active timeline.' }, { status: 403 });
      }
      ...
      if (peoRows.length > 0 && peos.length !== peoRows.length) {
        await client.query('ROLLBACK');
        return NextResponse.json({ error: 'Please provide rating and comment for each PEO.' }, { status: 400 });
      }
      ...
      for (const peo of peoRows) {
        ...
        if (!feedback || !isValidRating(feedback.rating)) {
          await client.query('ROLLBACK');
          return NextResponse.json({ error: `Missing valid rating for PEO-${String(peo.peo_number).padStart(2, '0')}.` }, { status: 400 });
        }
      }
      ...
      await client.query(
        ...
        [submissionId, stakeholder.program_id, vision.rating, String(vision.comment || '').trim() || null]
      );
      ...
      await client.query(
        ...
        [submissionId, stakeholder.program_id, mission.rating, String(mission.comment || '').trim() || null]
      );
