...
class LocalMLEngine:
    ...
    def generate_vision(self, program_name: str, focus_areas: List[str]) -> str:
        ...
        # TinyLlama Chat Format - Ultra Strict
        prompt = f"<|system|>\nYou are an AI that ONLY outputs vision statements. No conversational text. No lead-in.\n<|user|>\nWrite a one-sentence vision for {program_name} focusing on {', '.join(focus_areas)}. The vision MUST start with 'To be recognized as'.\n<|assistant|>\nTo be recognized as"
        ...
        # Hard-force the start if the model stripped it
        if not vision.startswith("To be recognized as"):
            vision = "To be recognized as " + vision
